...
static void test_pt1950_job(void)
{
    ...
    assert(ptouch_print_build_job_for_model(profile, &blank, 1, 1,
                                            &opts, &job));
    assert(job.chain);
    assert(count_exact_frames(&job, no_chain, sizeof no_chain) == 0);
    assert(job.stream.data[job.stream.len - 1] == PTOUCH_PRINT_CHAIN);
    ptouch_print_job_free(&job);
}
...
static void test_media_and_profile_validation(void)
{
    ...
    assert(pt1950->support == PTOUCH_SUPPORT_HARDWARE_VALIDATED);
    ...
    for (size_t i = 0; i < ptouch_model_count(); ++i) {
        ...
        if (candidate->completion_validated) {
            ...
            assert(candidate->pid == 0x20AF || candidate->pid == 0x2019);
        }
    ...
    assert(completion_validated == 2 && chain_validated == 2);
